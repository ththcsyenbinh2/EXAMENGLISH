...
import { supabase, isSupabaseConfigured, isApiKeyConfigured } from './services/supabase';
import { 
  ...
  ClipboardList, LayoutDashboard, AlertTriangle, Info
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isApiKeyConfigured()) {
      alert("Lỗi: API_KEY chưa được thiết lập trong Environment Variables. Vui lòng cấu hình trên Vercel/Cloud.");
      return;
    }

    setIsProcessing(true);
    setLoadingStep('Đang đọc file .docx...');
    try {
      ...
      <header className="bg-white border-b border-slate-200 py-4 px-8 sticky top-0 z-[100] backdrop-blur-md bg-white/80">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => window.location.hash = ''}>
            <div className="bg-indigo-600 p-2 rounded-xl shadow-lg"><GraduationCap className="text-white" size={24}/></div>
          ...
          <div className="flex items-center gap-4">
            ...
            {(!isApiKeyConfigured() || !isSupabaseConfigured()) && (
              <div className="flex items-center gap-2 bg-amber-50 text-amber-600 px-3 py-1.5 rounded-lg text-xs font-bold border border-amber-200">
                <AlertTriangle size={14}/> <span>Chưa cấu hình API</span>
              </div>
            )}
          </div>
        </div>
      ...
      <main className="py-8 px-6">
        {isProcessing && (
          <div className="max-w-xl mx-auto p-12 bg-white rounded-3xl text-center shadow-xl border border-slate-100 animate-pulse mb-8">
            ...
            <p className="text-slate-500 text-sm mt-2 italic">Hệ thống đang xử lý dữ liệu...</p>
          </div>
        )}
          ...
          <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                ...
                <p className="text-slate-500 font-medium">Hệ thống tạo đề từ Word và chấm điểm trực tuyến.</p>
              </div>
              <div className="flex gap-3">
                <button onClick={() => window.location.hash = 'hocsinh'} className="bg-white border border-slate-200 px-5 py-3 rounded-2xl font-bold hover:bg-slate-100 transition-all flex items-center gap-2">
                  <UserCircle size={20}/> Chế độ Học sinh
                </button>
                <label className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 cursor-pointer shadow-lg shadow-indigo-100 transition-all">
            ...
            {(!isApiKeyConfigured() || !isSupabaseConfigured()) && (
              <div className="bg-indigo-50 border border-indigo-100 p-6 rounded-3xl flex gap-4 items-start">
                <div className="bg-indigo-600 p-2 rounded-lg text-white"><Info size={20}/></div>
                <div>
                  <h4 className="font-bold text-indigo-900">Hướng dẫn cấu hình (Để app không bị lỗi)</h4>
                  <p className="text-indigo-700 text-sm mt-1">Bạn cần nạp các biến <strong>API_KEY</strong>, <strong>SUPABASE_URL</strong>, <strong>SUPABASE_ANON_KEY</strong> vào Environment Variables của Vercel/Hosting rồi <strong>Redeploy</strong> lại.</p>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {exams.map(exam => (
                <div key={exam.id} className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm hover:shadow-md transition-all group overflow-hidden relative">
                  ...
                  <h3 className="text-lg font-bold text-slate-900 mb-6 line-clamp-2 h-14">{exam.title}</h3>
                  ...
                  <div className="space-y-2">
                    <button onClick={() => { setCurrentExam(exam); setMode(AppMode.VIEW_SUBMISSIONS); }} className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-black">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={async () => { if(confirm("Xóa đề này?")) { await supabase.from('exams').delete().eq('id', exam.id); fetchInitialData(); } }} className="p-2.5 bg-red-50 text-red-500 rounded-xl hover:bg-red-100 border border-red-50">
                        <Trash2 size={16}/>
                      </button>
                    </div>
                  </div>
              ...
              {exams.length === 0 && !isDbLoading && (
                <div className="col-span-full py-20 bg-white rounded-[32px] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-slate-400">
                  ...
                  <p className="font-bold">Chưa có đề thi nào. Hãy tải lên file Word để bắt đầu!</p>
                </div>
              )}
            </div>
          </div>
        ...
        {mode === AppMode.STUDENT_ENTRY && (
          <div className="max-w-md mx-auto py-12 animate-fade-in">
            <div className="bg-white p-8 rounded-[40px] shadow-xl border border-slate-100">
              ...
              <h2 className="text-2xl font-black text-center mb-8">Phòng Thi Online</h2>
              <div className="space-y-4 mb-8">
                <input type="text" placeholder="Họ và tên của em" className="w-full p-4 rounded-xl bg-slate-50 border-2 border-transparent focus:border-indigo-600 outline-none font-bold" value={studentName} onChange={e => setStudentName(e.target.value)} />
                <input type="text" placeholder="Lớp (Ví dụ: 12A1)" className="w-full p-4 rounded-xl bg-slate-50 border-2 border-transparent focus:border-indigo-600 outline-none font-bold" value={className} onChange={e => setClassName(e.target.value)} />
                <input type="text" placeholder="MÃ ĐỀ THI" className="w-full p-4 rounded-xl bg-indigo-50 border-2 border-indigo-200 focus:border-indigo-600 outline-none font-black text-indigo-600 text-center uppercase tracking-widest text-xl" value={examCodeInput} onChange={e => setExamCodeInput(e.target.value)} />
              </div>
              <button onClick={async () => {
                if (!studentName || !className || !examCodeInput) return alert("Nhập đủ thông tin!");
                setIsDbLoading(true);
                ...
                } else alert("Mã đề không tồn tại hoặc đang đóng!");
                setIsDbLoading(false);
              }} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-5 rounded-2xl font-black text-lg flex items-center justify-center gap-2 active:scale-95 transition-all">
                VÀO THI <ChevronRight size={20}/>
              </button>
            </div>
          </div>
        )}
          ...
          <div className="max-w-3xl mx-auto space-y-8 animate-fade-in">
            ...
            <div className="space-y-6 pb-20">
              {currentExam.questions.map((q, idx) => (
                <div key={q.id} className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {q.options.map((opt, oIdx) => (
                      <button key={oIdx} onClick={() => setStudentAnswers({...studentAnswers, [q.id]: oIdx})} className={`p-4 rounded-xl border-2 text-left font-bold transition-all flex items-center gap-3 ${studentAnswers[q.id] === oIdx ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-100 text-slate-600 hover:bg-slate-50'}`}>
              ...
              }} className="w-full bg-emerald-500 hover:bg-emerald-600 text-white py-6 rounded-3xl font-black text-xl shadow-lg transition-all active:scale-95">NỘP BÀI THI</button>
            </div>
          </div>
        )}
        ...
        {mode === AppMode.STUDENT_RESULT && currentSubmission && (
          <div className="max-w-md mx-auto py-12 animate-fade-in text-center">
            <div className="bg-white p-10 rounded-[48px] shadow-2xl border border-slate-50">
              ...
              <button onClick={() => window.location.reload()} className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold hover:bg-black">Quay lại trang chính</button>
            </div>
          </div>
        )}
        ...
        {mode === AppMode.EXAM_SETUP && currentExam && (
          <div className="max-w-4xl mx-auto space-y-6 animate-fade-in pb-20">
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 flex justify-between items-center">
              <div>
                <h2 className="text-2xl font-black">Kiểm tra kết quả AI</h2>
                <p className="text-slate-500 text-sm">AI đã tìm thấy {currentExam.questions.length} câu hỏi trắc nghiệm.</p>
              </div>
              <button onClick={saveExam} className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-4 rounded-2xl font-bold flex items-center gap-2 shadow-lg transition-all active:scale-95">
              ...
              {currentExam.questions.map((q, idx) => (
                <div key={q.id} className="bg-white p-8 rounded-[32px] border border-slate-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {q.options.map((opt, oIdx) => (
                      <div key={oIdx} className={`p-4 rounded-xl border ${oIdx === q.correctAnswerIndex ? 'bg-emerald-50 border-emerald-500 text-emerald-700 font-bold' : 'bg-slate-50 border-transparent text-slate-500'}`}>
        ...
        {mode === AppMode.VIEW_SUBMISSIONS && currentExam && (
          <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-20">
            <button onClick={() => setMode(AppMode.TEACHER_DASHBOARD)} className="flex items-center gap-2 text-slate-400 hover:text-indigo-600 font-bold mb-4"><ArrowLeft size={18}/> Quay lại</button>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-slate-50 text-slate-400 text-[10px] font-black uppercase tracking-widest">
                      ...
                      <th className="px-8 py-5 text-right">Nộp bài</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    ...
                    {submissions.filter(s => s.exam_id === currentExam.id).length === 0 && (
                      <tr><td colSpan={5} className="text-center py-20 text-slate-400 font-bold italic">Chưa có kết quả.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
